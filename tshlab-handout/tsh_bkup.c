    ...
    */
    ...
    if (!builtin_command(&tok))  
    {
         if ((pid=Fork()) == 0 ) 
         {
                 ...
                 /*Unblocking all the signals for child process*/
         ...
         if (!bg) 
         {      
                 PRINTF("Waiting for the foreground process to complete\n");
...
/*
 * builtin_command - Check & execute builtin-command if it exists
 *
 * Parameters : 
...
{ 
         enum  builtins_type {       /* Indicates if argv[0] is a builtin command */
                         BUILTIN_NONE,
                         BUILTIN_QUIT,
                         BUILTIN_JOBS,
                         BUILTIN_BG,
...
sigchld_handler(int sig) 
{
    ...
    while((pid1 = waitpid(-1,&status,WNOHANG|WUNTRACED)) > 0 ) 
    {
           ...
           if (WIFSTOPPED(status)) 
            ...
            {
                    PRINTF("Unable to delete from the job list\n");
            }
            
    }
    if (errno != ECHILD && !WIFSTOPPED(status) && !WIFEXITED(status) && !WIFSIGNALED(status))  
    { 
            fprintf(stdout,"Errors are errono : %s, WIFSTOPPED : %d, WIFEXITED : %d, WIFSIGNALED : %d", strerror(errno),WIFSTOPPED(status),WIFEXITED(status),WIFSIGNALED(status));
            fprintf(stdout,"Waitpid Error Ish");
            PRINTF("Waitpid error\n");
    }
    
    /* Removing from the job list */
