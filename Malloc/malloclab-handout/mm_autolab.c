...
#define CHUNKSIZE   256 /* Extend heap by this amount (bytes) */  //line:vm:mm:endconst 
...
static void updateList() 
{
        ...
        dbg_printf("prev bucket is %d and bucket points to %p\n",bucket_number,GETPTR(BUCKET_ELEM(bucket_number)));
        dbg_printf("Exiting update list and bucket_number set to %p\n",GETPTR(BUCKET_ELEM(bucket_number)));
}
