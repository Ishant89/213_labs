...
#define SETNEXT(p)   (*(unsigned int *)(p) = ((GET(p) | 2)))
#define UNSETNEXT(p)   (*(unsigned int *)(p) = ((GET(p) & (~2))))
        ...
        /* $end mmfree */
            ...
            /* $begin mmfree */
                ...
                /* $end mmfree */
                  ...
                  dbg_printf("blk is %p is and header is  %x\n",bp,GET(HDRP(bp))); 
                    if(GET(HDRP(bp)) & 2 ) 
                    {
                        ...
                        WRITEPREDESSOR(bp,0);
                  dbg_printf("blk is %p is and header is  %x\n",bp,GET(HDRP(bp))); 
                  dbg_printf("prev blk is %p is and header is  %x\n",PREV_BLKP(bp),GET(HDRP(PREV_BLKP(bp)))); 
                  dbg_printf("Header of free next block before %p is  %x\n",bp,GET(HDRP(NEXT_BLKP(bp)))); 
                        UNSETNEXT(HDRP(NEXT_BLKP(bp)));
                  dbg_printf("Header of free next block %p is  %x\n",bp,GET(HDRP(NEXT_BLKP(bp)))); 
...
static void *coalesce(void *bp) 
{
        ...
        dbg_printf("previous allocated block check is %ld and prev blk is %p and header is %x\n", prev_alloc,PREV_BLKP(bp),GET(HDRP(bp)));
...
static void place(void *bp, size_t asize)
             /* $end mmplace-proto */
{
        dbg_printf("Entering place %p,%ld\n",bp,asize);
            size_t csize = GET_SIZE(HDRP(bp));   
        dbg_printf("csize is  %ld\n", csize);
            findList(csize);
      ...
      /*      if ((NEXTFREE(bp) == NULL) && (PREVFREE(bp) == NULL)) 
            ...
            if ((csize - asize) >= (2*DSIZE)) { 
                  ...
                  dbg_printf("Header of allocated block %p is  %x\n",bp,GET(HDRP(bp))); 
                    PUT(FTRP(bp), PACK(asize, 1));
                    bp = NEXT_BLKP(bp);
                  dbg_printf("Header of split block %p is  %x\n",bp,GET(HDRP(bp))); 
                  ...
                  dbg_printf("Header of split block %p is  %x\n",bp,GET(HDRP(bp))); 
                   ...
                   } else 
                   {
                    ...
                    SETNEXT(HDRP(NEXT_BLKP(bp)));
      //              PUT(HDRP(NEXT_BLKP(bp)), PACK(csize, 2));
                  dbg_printf("Block is not split\n"); 
                  dbg_printf("Header of not split block %p is  %x\n",bp,GET(HDRP(bp))); 
